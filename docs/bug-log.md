...
| AVL-1 | P1 | Worker runs jobs as fire-and-forget subprocess; killed on every deploy/scale-in | Availability / lifecycle | Verified | **Partially resolved** — this PR, ADR 0017. Hardening subset only (concurrency cap, Python memory limit, honest Terraform comment); the strategic execution-model rework (pull-worker / Cloud-Run-Jobs-per-run) remains open |
...
### AVL-1 — Worker execution model fights Cloud Run lifecycle — **Partially resolved**
...
**Resolution (partial, hardening subset per explicit scoping decision):** `createWorkerApp` gains `maxConcurrency` (env `WORKER_MAX_CONCURRENCY`, default 10); `/execute` returns `503 {retryable: true}` once the in-flight count reaches it — reusing the existing `activeExecutions` tracking, no new structure. `resolveSpawnCommand`'s Python branch now enforces a real memory cap via `bash -c 'ulimit -v <kb>; exec "$0" "$@"' <python> <entrypoint>` (argv-passed, not string-interpolated — injection-safe despite the shell hop; `exec` keeps the same pid so kill/timeout logic is unaffected) — verified empirically that a 64MB limit is sufficient for normal startup while reliably killing a 200MB allocation. The Terraform `timeout` comment now accurately describes what Cloud Run's per-request timeout does and doesn't guarantee (comment-only, no functional Terraform change). See `docs/adr/0017-worker-hardening-subset.md`. **Residual (explicitly deferred, strategic rework unchanged):** the 202-then-background-subprocess pattern itself, true per-job isolation, and `maxDrainMs`'s actual reliability during scale-in all remain open — only the pull-worker/Cloud-Run-Jobs-per-run rework closes those.
