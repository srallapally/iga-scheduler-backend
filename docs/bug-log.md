...
| SEC-4 | P1 | Same-UID co-residency: job subprocess can read broker `/proc/1/environ` | Security / isolation | By inspection | **Partially resolved** — this PR, ADR 0009. DB password moved to Secret Manager fetch (no env-var secrets left to leak via `/proc/1/environ` today); job-UID separation (`gosu`/`jobrunner`) deferred to AVL-1 — could not be verified in a real container in this environment, and container-per-job supersedes the question anyway |
