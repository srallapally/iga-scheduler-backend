...
export class JobInstanceService {
  constructor({ esClient = createEsClient(), config = getConfig(), definitionsIndex = config.definitionsIndex, instancesIndex = config.instancesIndex } = {}) {
    this.es = esClient;
  ...
  async createInstance(definitionId, body) {
    ...
    const doc = { instanceId: parsed.instanceId, definitionId, definitionVersion: definition.version, definitionParameterSchema: parameterSchema, enabled: parsed.enabled, state: parsed.enabled ? "ACTIVE" : "PAUSED", schedule: parsed.schedule, nextFireAt, lastFireAt: null, parameters: parsed.parameters, createdAt: now, updatedAt: now };
    await this.es.create({ index: this.instancesIndex, id: parsed.instanceId, document: doc, refresh: true });
    return doc;
  }
  ...
  async patchInstance(instanceId, body) {
    ...
    await this.es.update({ index: this.instancesIndex, id: instanceId, doc: next, refresh: true });
    return next;
  }
  ...
  async deleteInstance(instanceId) { const existing = await this.getInstance(instanceId); const deleted = { ...existing, enabled: false, state: "DELETED", updatedAt: new Date().toISOString() }; await this.es.update({ index: this.instancesIndex, id: instanceId, doc: deleted, refresh: true }); return deleted; }
  async getInstance(instanceId) { const response = await this.es.get({ index: this.instancesIndex, id: instanceId }); return response._source; }
  async getActiveDefinition(definitionId) { const response = await this.es.get({ index: this.definitionsIndex, id: definitionId }); const definition = response._source; if (definition.enabled !== true || definition.state !== "ACTIVE") throw new Error(`Job definition ${definitionId} is not active`); return definition; }
  ...
  validateParameterValue(parameter, supplied) {
    ...
    if (parameter.type === "sensitive") { if (!supplied.secretRef || typeof supplied.secretRef !== "string") throw new Error(`Parameter ${parameter.name} must include secretRef`); return; }
